...
def equalize_loss(pred,
    ...
    try:
        assert(category_num != None)
    except:
        print("Error! Use EQL that need to specify the category num(remember to contain the bg class, and bg's label is zero)")
    for k in range(1,category_num):
        class_weight[:,k][(sample_index*(temp_label!=k)).view(label.shape)] = 0.0
...
class CrossEntropyLoss(nn.Module):
    ...
    def __init__(self,
        ...
        assert int(use_sigmoid) + int(use_mask) + int(use_eql) < 2
