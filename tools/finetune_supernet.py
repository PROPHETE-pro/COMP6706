...
from mmseg.datasets import (build_dataloader, replace_ImageToTensor)
...
import gaiaseg
from gaiaseg.datasets import build_dataset
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train a segmentor from supernet')
    ...
    parser.add_argument('--model-space-path', dest='model_space_path',
                        default=None, help='model space path')
    parser.add_argument('--tmpdir', help='the tmp dir to store results')
    parser.add_argument('--metric-tag', default='finetune',
...
def main():
    ...
    '''
    # mmseg 0.8.0 does not support replace_ImageToTensor
        ...
        try:
            ...
            if 'datasets' in cfg.data.test.keys():
                for ds in cfg.data.test.datasets:
                    ds.pipeline = DefaultFormatBundle(ds.pipeline)
            else:
    ...
    for i, model_meta in enumerate(model_metas):
        ...
        model_sample_cfg = {
            'type': 'anchor',
            'anchors': [{'name': str(i), **unfolded_model_meta}],
        }
        ...
        anchor_name = model_meta.get('name', model_meta.get('index', 'n'+str(i)))
